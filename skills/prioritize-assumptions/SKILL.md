...
description: "Prioritize assumptions using an Impact x Risk matrix and suggest experiments for each. Use when triaging a list of assumptions, deciding what to test first, or applying the assumption prioritization canvas."
---

## Prioritize Assumptions

Triage assumptions using an Impact x Risk matrix and suggest targeted experiments.
...
**ICE** works well for assumption prioritization: Impact (Opportunity Score x # Customers) x Confidence (1-10) x Ease (1-10). Opportunity Score = Importance x (1 - Satisfaction), normalized to 0-1 (Dan Olsen). **RICE** splits Impact into Reach x Impact separately: (R x I x C) / E. See the `prioritization-frameworks` skill for full formulas and templates.
...
1. **For each assumption**, evaluate two dimensions:
   - **Impact**: The value created by validating this assumption AND the number of customers affected (in ICE: Impact = Opportunity Score x # Customers)
   - **Risk**: Defined as (1 - Confidence) x Effort

2. **Categorize each assumption** using the Impact x Risk matrix:
